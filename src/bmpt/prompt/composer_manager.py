...
class Composer:
    """提示词组合器，用于将多个输出片段与预设提示拼接。

    Attributes:
        name: composer 名称
        prompt_token_ids: 预 tokenize 的提示段列表，长度为 outputs 数量 + 1
        pad_token_id: 填充 token ID
        max_total_len: 最大总长度限制
        truncate_side: 截断方向，"left" 或 "right"
        pad_to_multiple_of: 可选，将长度填充到该数的倍数
        output_pad_token_id: 可选，识别输出有效长度的 pad token
    """
    name: str
    ...
    def compose(
        ...
        Args:
            outputs: 模型输出列表，每个 tensor 形状为 [B, T]
            output_masks: 可选，每个输出的有效位置掩码

        Returns:
            包含 input_ids、attention_mask、lengths 的字典
        """
        ...
        output_pad = self.pad_token_id if self.output_pad_token_id is None else self.output_pad_token_id
        prompt_lens = [int(p.numel()) for p in self.prompt_token_ids]
        ...
        for idx, output in enumerate(outputs):
            if output_masks is not None:
                ...
                lens = (output != output_pad).to(dtype=torch.long).sum(dim=1)
        ...
        target_len = min(observed_max_len, self.max_total_len) if self.max_total_len > 0 else observed_max_len
        ...
        # 将 prompts 移到目标设备
        prompts_on_device = [
            p.to(device=device, dtype=outputs[0].dtype) for p in self.prompt_token_ids
        ]
...
def _load_tokenizer_for_prompting(config: dict[str, Any]) -> tuple[Any, int]:
    ...
    tokenizer_path = resolve_tokenizer_source(config, local_source=local_source)
...
def build_composers_from_config(config: dict[str, Any]) -> dict[str, Composer]:
    ...
    for name, composer_cfg_raw in composers_cfg.items():
        ...
        if not isinstance(prompts, list) or len(prompts) < 2:
            raise ValueError(
                f"prompting.composers.{name}.prompts must contain at least 2 prompt segments"
            )
        ...
        prompt_token_ids = [
            _tokenize_prompt(tokenizer, str(text), add_bos=add_bos, add_eos=add_eos)
            for text in prompts
        ]
        ...
        composers[name] = Composer(
            ...
            pad_to_multiple_of=(
                None if composer_cfg.get("pad_to_multiple_of") is None
                else int(composer_cfg.get("pad_to_multiple_of"))
            ),
